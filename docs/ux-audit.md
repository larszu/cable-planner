...
32. 🔴 `[~]` **15 hand-rolled `fixed inset-0` modals without `useDialogA11y`** (no focus trap, Escape, or focus return). **Migrated to `ModalShell` (focus-trap/Escape/focus-return + standard close):** `NonRackAddDialog`, `PatchPanelCreateDialog`, `RackShelfCreateDialog`, `LocationBomDialog`, `MobileShareDialog`. **Still hand-rolled** (multi-region tables, multi-column layouts, or custom keyboard handling — need per-dialog restructuring + visual verification): `CableBomDialog`, `RackEditorDialog`, `GraphmlImportDialog`, `VideohubExportDialog`, `RentmanImportDialog`, `RentmanCableExportDialog`, `AtemAudioRouterDialog`, `AtemMvConfigDialog`, `RackImageCropDialog` (custom crop keyboard), `GreenGoExportDialog`.
...
49. 🔴 `[~]` **`ModalShell` exists but most large dialogs ignored it** (hand-rolled chrome, differing `max-w`, close button, drag, a11y). **+5 migrated** this pass (NonRackAdd/PatchPanel/RackShelf/LocationBom/MobileShare); the complex table/multi-column/tab dialogs remain (tracked under #32).
...
| 5 | **a11y baseline**: native dialogs → custom; 15 modals → `ModalShell`; `aria-label` on icon buttons | #32, #33, #41 | 🔨 native dialogs done (#41); icon-button `aria-label` swept (#33/#37/#38/#40); 5/15 modals → ModalShell (#32/#49/#55); 10 complex dialogs remain |
