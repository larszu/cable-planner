...
41. 🔴 `[x]` **Native browser dialogs despite a custom system**: `App.tsx` `confirm()`, `LayerVisibilityChips.tsx` `confirm()`, `RackBuilderDialog.tsx` ×2 `alert()` → **all replaced** with `confirmDialog`/`infoDialog` (themed, a11y, consistent). **Correction (2026-09-03):** this line already claimed „no native `confirm/alert/prompt` left in components" while `CollabPanel.tsx:103` still called `window.confirm` — and it guarded the one action that replaces the local plan, so it was also the only one that needed `destructive: true`. Now `confirmDialog`. The claim is no longer prose: `tests/nativeDialogs.test.ts` computes the call sites from source and names any new one by file and line.
