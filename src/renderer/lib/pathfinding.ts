...
// Spatial-index bucket size (grid cells).
const PENALTY_BUCKET = 10;
// Hash-key constants for (bucketCol, bucketRow) → number.
// Inputs are first shifted by HASH_OFFSET so negative origins don't break the
// encoding; HASH_STRIDE must comfortably exceed (2 * HASH_OFFSET) so the two
// coordinates don't overlap. Result stays well within JS safe-integer range
// (< 2^53) for any sane canvas.
const HASH_OFFSET = 1 << 15;        // 32768
const HASH_STRIDE = 1 << 17;        // 131072
const packBucketKey = (bc: number, br: number) =>
  (bc + HASH_OFFSET) * HASH_STRIDE + (br + HASH_OFFSET);

// U-turn cost multipliers (relative to TURN_PENALTY).
const UTURN_MID_PATH_MULT = 5;      // mid-path 180° reversal
const UTURN_FROM_START_MULT = 10;   // discourages starting a search by reversing the forced exit direction
 ...
 *
 *  NOTE: The heuristic adds TURN_PENALTY when a turn is mandatory to reach
 *  the goal, which is admissible *only* when those turns truly are required.
 *  For the orthogonal grids we build it remains tight in practice, but
 *  optimality is not formally guaranteed — small detours can occasionally
 *  beat a more direct path. Trade-off chosen for speed. */
export function astarOrthogonal(
  ...
  if (freeStartDir) {
    // excludeStartDir gets a heavy U-turn penalty rather than being banned
    // outright — keeps it usable as a fallback if no better start exists.
    const UTURN_COST = TURN_PENALTY * UTURN_FROM_START_MULT;
  ...
  while (open.length > 0) {
    ...
    if (current.xi === eci && current.yi === eri) {
      ...
      // Goal cell reached from a rejected arrival direction: don't mark
      // this state closed, just fall through and keep expanding so other
      // approach directions still have a chance.
    }
    ...
    for (let d = 0; d < 4; d++) {
      ...
      if (d !== current.dir && current.dir >= 0) {
        ...
        let turnCost = isUturn ? TURN_PENALTY * UTURN_MID_PATH_MULT : TURN_PENALTY;
      ...
      if (penaltyGrid) {
        ...
        const bucket = penaltyGrid.get(packBucketKey(bc, br));
        if (bucket) {
...
export function computeEdgePath(
  ...
  // Label sits at the midpoint of the longest segment so it lands on a
  // clear stretch rather than near a bend.
  let labelX = (simplified[0].x + simplified[simplified.length - 1].x) / 2;
  let labelY = (simplified[0].y + simplified[simplified.length - 1].y) / 2;
  let bestLen = -1;
