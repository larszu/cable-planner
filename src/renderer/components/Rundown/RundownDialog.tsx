...
export const RundownDialog = () => {
  ...
  const einlesen = () => {
    ...
    const naechster: RundownPlan = {
      source: quelle.trim() || t('rundown.sourceUnknown', 'Herkunft unbekannt'),
  ...
  return (
    <ModalShell
      ...
      title={t('rundown.title', 'Ablauf und Kamera-Aufträge')}
      ...
      <div className="space-y-4 text-cp-sm">
        <section className="rounded border border-cp-border bg-cp-surface-2/40 p-3">
          <h3 className="mb-1 font-medium text-cp-text">
            {t('rundown.import', 'Ablauf einlesen')}
          </h3>
          <PanelHint
          ...
          <div className="mb-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
            <label className="block text-cp-xs text-cp-text-secondary">
              <span className="mb-1 block">{t('rundown.source', 'Herkunft')}</span>
              <input
                value={quelle}
                onChange={(e) => setQuelle(e.target.value)}
                placeholder={t('rundown.source.placeholder', 'z. B. Regieplan.xlsx, Mail vom 9.9.')}
                className="w-full rounded border border-cp-border bg-cp-surface-1 p-2"
              />
            </label>
            <label className="block text-cp-xs text-cp-text-secondary">
              <span className="mb-1 block">{t('rundown.revision', 'Stand der Quelle')}</span>
              <input
                value={stand}
                onChange={(e) => setStand(e.target.value)}
                placeholder={t('rundown.revision.placeholder', 'z. B. v4, Fassung 3')}
                className="w-full rounded border border-cp-border bg-cp-surface-1 p-2"
              />
            </label>
          </div>
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            >
              {t('rundown.read', 'Einlesen')}
            </button>
            {rundown && (
              <span className="text-cp-xs text-cp-text-muted">
                {rundown.segments.length} {t('rundown.segments', 'Abschnitte')} ·{' '}
                {rundown.source}
            ...
            <ul className="mt-2 flex flex-col gap-1">
              {funde.map((f) => (
                <li key={f.zeile} className="text-cp-xs text-cp-warn">
                  {t('rundown.finding.line', 'Zeile')} {f.zeile}:{' '}
                  {f.grund === 'ohne-titel'
                    ? t('rundown.finding.noTitle', 'Nummer ohne Titel — nicht übernommen')
                    : t('rundown.finding.empty', 'nichts Lesbares — nicht übernommen')}{' '}
        ...
        {rundown && identities.length > 0 && (
          <section>
            <h3 className="mb-1 font-medium text-cp-text">
              {t('rundown.coverage', 'Wer macht was')}
            </h3>
            <PanelHint
              className="mb-2 text-cp-xs text-cp-text-muted"
              text={t(
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-cp-xs">
                <thead className="text-cp-text-secondary">
                  <tr>
                    <th className="px-2 py-1 text-left">{t('rundown.col.no', 'Nr.')}</th>
                    <th className="px-2 py-1 text-left">{t('rundown.col.title', 'Abschnitt')}</th>
                    {identities.map((i) => (
