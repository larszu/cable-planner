...
export const CrewTab = ({ projectName }: { projectName: string }) => {
  ...
  const zusageUebernehmen = () => {
    ...
    addApproval({
      ...
      by: beste.by ?? t('analysis.crew.unknownBy', 'sender not stated'),
      text: beste.text,
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={exportCsv} className={knopf}>
          <Icon icon={Download} size="xs" /> {t('analysis.crew.csv', 'Sheet as CSV')}
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 rounded border border-cp-border bg-cp-surface-2 p-2 text-cp-xs">
        ...
        <span>
          {t('analysis.crew.labour', 'Labour')}: <strong>{abrechnung.labourTotal.toFixed(2)}</strong>
        </span>
        <span>
          {t('analysis.crew.expensesBillable', 'Billable expenses')}:{' '}
          <strong>{abrechnung.expensesBillable.toFixed(2)}</strong>
        </span>
        <span className="text-cp-text-muted">
          {t('analysis.crew.expensesOwn', 'own expenses')}:{' '}
          {abrechnung.expensesOwn.toFixed(2)}
        </span>
        <span>
          {t('analysis.crew.billable', 'to the client')}:{' '}
          <strong>{abrechnung.billableTotal.toFixed(2)}</strong>
        </span>
        {geplant > 0 && (
          <span className="text-cp-text-muted">
            {format(
              t('analysis.crew.planned', '{n} pencilled/held — not in the totals'),
              { n: geplant },
            )}
          </span>
        )}
        {abrechnung.unpricedEntries > 0 && (
          <span className="text-amber-300">
            {format(
              t('analysis.crew.unpriced', '{n} shift(s) without a rate — not in any total'),
      ...
      <section className="flex flex-col gap-1">
        <h3 className="text-cp-xs font-semibold text-cp-text">{t('analysis.crew.people', 'People')}</h3>
        {plan.people.map((p) => (
      ...
      {/* ── Zuschlagsbänder ── */}
      <section className="flex flex-col gap-1">
        <h3 className="text-cp-xs font-semibold text-cp-text">{t('analysis.crew.bands', 'Surcharge bands')}</h3>
        {plan.bands.map((b) => (
      ...
      {/* ── Sätze ── */}
      <section className="flex flex-col gap-1">
        <h3 className="text-cp-xs font-semibold text-cp-text">{t('analysis.crew.rates', 'Rates')}</h3>
        {plan.rates.map((r) => (
          <div key={r.id} className="flex flex-wrap items-center gap-1.5 text-cp-xs">
            ...
            <input
              ...
              placeholder={t('analysis.crew.calloutPh', 'Call-out')}
              aria-label={t('analysis.crew.callout', 'Call-out fee')}
            ...
            <input
              ...
              placeholder={t('analysis.crew.otAfterPh', 'after h')}
              aria-label={t('analysis.crew.otAfter', 'Overtime after hours')}
            ...
            <input
              ...
              aria-label={t('analysis.crew.otPercent', 'Overtime surcharge')}
              className={`${inp} w-[4.5rem] tabular-nums`}
            />
            {plan.bands.map((b) => (
      ...
      {/* ── Schichten ── */}
      <section className="flex flex-col gap-1">
        <h3 className="text-cp-xs font-semibold text-cp-text">{t('analysis.crew.entries', 'Shifts')}</h3>
        {plan.entries.map((e) => {
          ...
          return (
            <div key={e.id} className="flex flex-wrap items-center gap-1.5 text-cp-xs">
              <input
                ...
                aria-label={t('analysis.crew.date', 'Date')}
                className={inp}
              />
              <select
                value={e.rateId}
                ...
                aria-label={t('analysis.crew.rate', 'Rate')}
                className={inp}
              >
                {plan.rates.map((r) => (
              ...
              <input
                ...
                aria-label={t('analysis.crew.start', 'Start')}
                className={`${inp} w-[4.5rem] tabular-nums`}
              />
              ...
              <input
                ...
                aria-label={t('analysis.crew.end', 'End')}
                className={`${inp} w-[4.5rem] tabular-nums`}
              />
              ...
              <select
                ...
                aria-label={t('analysis.crew.booking', 'Booking state')}
                className={inp}
              >
              ...
              <span className="tabular-nums text-cp-text-muted">
                ...
                {e.endMinute > 1440 ? ` (${t('analysis.crew.nextDay', 'next day')})` : ''}
              </span>
              {c && (
                <span className="tabular-nums">
                  ...
                  {c.overtimeMinutes > 0
                    ? ` · ${formatHours(c.overtimeMinutes)} ${t('analysis.crew.ot', 'overtime')}`
                    : ''}
                </span>
              )}
              <button
        ...
        {plan.rates.length > 0 && (
          <button
            ...
            className={knopf}
          >
            <Icon icon={Plus} size="xs" /> {t('analysis.crew.addEntry', 'Shift')}
          </button>
        )}
      </section>
      ...
      {/* ── Auslagen ── */}
      <section className="flex flex-col gap-1">
        <h3 className="text-cp-xs font-semibold text-cp-text">{t('analysis.crew.expenses', 'Expenses')}</h3>
        <PanelHint
          text={t(
        ...
        {plan.expenses.map((x) => (
          <div key={x.id} className="flex flex-wrap items-center gap-1.5 text-cp-xs">
            ...
            <span className="text-cp-text-muted">
              {x.receiptRef ?? t('analysis.crew.noReceipt', 'no receipt')}
            </span>
            <BelegZelle
              expense={x}
              filePath={filePath}
            ...
            <span className={x.billable ? '' : 'text-cp-text-muted'}>
              {x.billable
                ? t('analysis.crew.billableYes', 'goes to the client')
                : t('analysis.crew.billableNo', 'own cost')}
            </span>
            <button
              type="button"
      ...
      {(kette.rows.length > 0 || kette.unlinked.length > 0) && (
        <section className="flex flex-col gap-1">
          <h3 className="text-cp-xs font-semibold text-cp-text">
            {t('analysis.crew.coverage', 'Receipt coverage per cost line')}
          </h3>
          <PanelHint
            text={t(
          ...
          {kette.rows.map((r) => (
            <div key={r.line.id} className="flex flex-wrap items-center gap-1.5 text-cp-xs">
              ...
              <span className="tabular-nums">
                {t('analysis.crew.documented', 'documented')}: {r.documented.toFixed(2)}
              </span>
              <span className="tabular-nums text-cp-text-muted">
                {t('analysis.crew.withFile', 'of which with a file')}: {r.withFile.toFixed(2)}
              </span>
              <span className="tabular-nums">
                {t('analysis.crew.actual', 'Actual')}:{' '}
                {r.line.actual === undefined
                  ? t('analysis.crew.actualUnset', 'not set')
            ...
            <p className="text-cp-xs text-cp-warn">
              {format(
                t(
                  ...
                  '{n} expenses point at no cost line and show up in no comparison.',
                ),
      ...
      {/* ── Zusagen (Bedarf 42) ── */}
      <section className="flex flex-col gap-1">
        <h3 className="text-cp-xs font-semibold text-cp-text">{t('analysis.crew.approvals', 'Approvals')}</h3>
        <PanelHint
          className="mb-1 text-cp-xs text-[var(--cp-text-muted)]"
          text={t(
        ...
        <textarea
          ...
          placeholder={t(
            ...
            '[09/09/26, 20:14] Max Mustermann: yes, do the overtime',
          )}
          aria-label={t('analysis.crew.paste', 'Paste message')}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <select
            ...
            aria-label={t('analysis.crew.scope', 'What the approval covers')}
            className={inp}
          >
            <option value="free">{t('analysis.crew.scopeFree', '— general —')}</option>
            {tageMitMehrarbeit.map(([key, label]) => (
              <option key={key} value={key}>
                {t('analysis.crew.scopeOvertime', 'Overtime')}: {label}
              </option>
            ))}
          </select>
          <button type="button" onClick={zusageUebernehmen} disabled={!paste.trim()} className={knopf}>
            <Icon icon={Plus} size="xs" /> {t('analysis.crew.addApproval', 'Record approval')}
          </button>
        </div>
        {plan.approvals.map((a) => (
          <div key={a.id} className="flex flex-wrap items-start gap-1.5 text-cp-xs">
            <span className="min-w-[9rem] tabular-nums text-cp-text-muted">
              {a.givenAt ?? t('analysis.crew.noGivenAt', 'time not stated')}
            </span>
            <span className="min-w-[8rem]">{a.by}</span>
            <span className="min-w-0 flex-1 whitespace-pre-wrap">{a.text}</span>
            <span className="text-cp-text-muted">
              {a.covers.kind === 'overtime'
                ? `${t('analysis.crew.scopeOvertime', 'Overtime')} ${a.covers.date}`
                : t('analysis.crew.scopeFree', '— general —')}
            </span>
            <button
              type="button"
      ...
      {uebergabe.length > 0 && (
        <section className="flex flex-col gap-1">
          <h3 className="text-cp-xs font-semibold text-cp-text">
            {t('analysis.crew.handoff', 'Handover to the accounts (net)')}
          </h3>
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      <input
        ...
        placeholder={t('analysis.crew.companyPh', 'Company / freelance')}
        aria-label={t('analysis.crew.company', 'Company')}
        className={`${inp} w-[10rem]`}
      />
      <button
        type="button"
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <input
        ...
        placeholder={t('analysis.crew.bandPh', 'Band name, e.g. Night')}
        aria-label={t('analysis.crew.bandName', 'Band name')}
        className={`${inp} w-[10rem]`}
      />
      <input
        value={von}
        onChange={(e) => setVon(e.target.value)}
        aria-label={t('analysis.crew.bandFrom', 'Band from')}
        className={`${inp} w-[4.5rem] tabular-nums`}
      />
      <input
        value={bis}
        onChange={(e) => setBis(e.target.value)}
        aria-label={t('analysis.crew.bandTo', 'Band to')}
        className={`${inp} w-[4.5rem] tabular-nums`}
      />
      <input
        value={prozent}
        onChange={(e) => setProzent(e.target.value)}
        aria-label={t('analysis.crew.bandPercent', 'Surcharge in percent')}
      ...
      <label className="flex items-center gap-1 text-cp-xs text-cp-text-muted">
        ...
        {t('analysis.crew.sundayOnly', 'Sundays only')}
      </label>
      <button
        type="button"
        disabled={!label.trim()}
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      <input
        ...
        placeholder={t('analysis.crew.activityPh', 'Activity')}
        aria-label={t('analysis.crew.activity', 'Activity')}
        className={`${inp} w-[8rem]`}
      />
      <input
        ...
        placeholder={t('analysis.crew.hourlyPh', 'Hourly rate')}
        aria-label={t('analysis.crew.hourly', 'Hourly rate')}
        className={`${inp} w-[6rem] tabular-nums`}
      />
      <button
...
const BelegZelle = ({
  ...
  if (!hasDesktopBridge) {
    return (
      <span className="text-cp-text-muted">
        {t('analysis.crew.receiptDesktopOnly', 'Receipt files only in the desktop app')}
      </span>
    )
  }
      ...
      {vorschlag && (
        <div className="flex flex-col gap-0.5 text-cp-xs">
          <div className="flex flex-wrap gap-2">
            <Gelesen label={t('analysis.crew.readDate', 'Date')} feld={vorschlag.date} />
            <Gelesen
              label={t('analysis.crew.readAmount', 'Amount')}
              feld={vorschlag.amount}
              zeige={(v) => v.toFixed(2)}
            />
            <Gelesen label={t('analysis.crew.readMerchant', 'Merchant')} feld={vorschlag.merchant} />
            <Gelesen label={t('analysis.crew.readCurrency', 'Currency')} feld={vorschlag.currency} />
            <Gelesen
              label={t('analysis.crew.readVat', 'Tax rate')}
          ...
          {vorschlag.amountCandidates.length > 1 && (
            <div className="flex flex-wrap items-center gap-1">
              <span className="text-cp-text-secondary">
                {t('analysis.crew.pickAmount', 'Which amount is the total?')}
              </span>
      ...
      <div className="flex flex-wrap items-center gap-1">
        ...
        {zeile && !zeile.ok && (
          <span className="text-cp-xs text-cp-text-muted">
            {format(t('analysis.crew.missingFields', 'Missing: {was}'), {
              was: zeile.missing
                .map((m) =>
                  m === 'amount'
                    ? t('analysis.crew.readAmount', 'Amount')
                    : t('analysis.crew.readDate', 'Date'),
                )
                .join(', '),
            })}
          </span>
...
const Gelesen = <T,>({
  ...
  if (!feld) {
    return (
      <span className="text-cp-text-muted">
        {label}: {t('analysis.crew.readNothing', 'not read')}
      </span>
    )
  }
  return (
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <input
        ...
        aria-label={t('analysis.crew.expenseDate', 'Date of the expense')}
        className={inp}
      />
      <select
        value={art}
        onChange={(e) => setArt(e.target.value as CrewExpense['kind'])}
        aria-label={t('analysis.crew.expenseKind', 'Kind of expense')}
        className={inp}
      >
      ...
      <input
        value={betrag}
        onChange={(e) => setBetrag(e.target.value)}
        placeholder={t('analysis.crew.amountPh', 'Amount')}
        aria-label={t('analysis.crew.amount', 'Amount')}
        className={`${inp} w-[6rem] tabular-nums`}
      />
      <input
        value={beleg}
        onChange={(e) => setBeleg(e.target.value)}
        placeholder={t('analysis.crew.receiptPh', 'Receipt')}
        aria-label={t('analysis.crew.receipt', 'Receipt')}
