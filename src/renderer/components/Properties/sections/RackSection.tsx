...
export const RackSection = ({ equipment }: { equipment: EquipmentItem }) => {
  ...
  return (
    <>
      <SortableSection
        id="rack"
        title={t('props.rack.title', 'Rack / 19" settings')}
        subtitle={
          equipment.isRackDevice
            ? format(t('props.rack.units', '{n} RU'), { n: equipment.rackUnits ?? 1 })
            : t('props.rack.inactive', 'inactive')
        }
      ...
      >
        <fieldset className="border-0 p-0">
          <label className="mb-2 flex items-center gap-2 text-cp-xs">
            ...
            />
            <span>{t('props.rack.isRack', 'Is a 19" rack device')}</span>
          </label>
          ...
          {equipment.isRackDevice && (
            <>
              <div className="grid grid-cols-2 gap-2">
                ...
                <label className="block">
                  <span className="mb-1 block text-cp-text-secondary">{t('props.rack.view', 'View')}</span>
                  <select
                    value={rackViewMode}
                  ...
                  >
                    ...
                    <option value="both">{t('props.rack.frontRear', 'Front + rear')}</option>
                  </select>
                </label>
              </div>
